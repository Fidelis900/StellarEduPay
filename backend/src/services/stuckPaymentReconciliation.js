...
async function reconcileStuckPayments() {
  ...
  for (const p of stuck) {
    try {
      await enqueueTransaction(p.txHash, { schoolId: p.schoolId, studentId: p.studentId });
      requeued++;
    } catch (err) {
      logger.error('Failed to re-queue stuck payment', { txHash: p.txHash, error: err.message });
    }
  }
